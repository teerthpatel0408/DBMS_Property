...
SELECT AADHAAR_ID, NAME, AGE, DOOR_NO, STREET, CITY, STATE, PINCODE FROM SYS.USERS WHERE AADHAAR_ID IN (SELECT AID FROM SYS.PROP_RENTED WHERE PID = propID AND START_DATE<(SELECT CURRENT_DATE from DUAL) AND END_DATE>(select CURRENT_DATE from DUAL));prop_id||CHR(9)||CHR(9)||floors||CHR(9)||t_area||CHR(9)||p_area||CHR(9)||construction_year||CHR(9)||door||CHR(9)||street||CHR(9)||city||CHR(9)||state||CHR(9)||pincode||CHR(9)||percent_hike||CHR(9)||rent||CHR(9)||prop_type||CHR(9)||total_bed
begin
